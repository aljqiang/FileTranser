...
public class QuartzScheduleInfoMap {
     ...
     * 添加调度任务
     */
    public static void addQS(long task_id,int task_type){
        synchronized(map){
            QuartzScheduleInfoMap.map.put(task_id,task_type);
        }
    }
     ...
     * 移除调度任务
     */
    public static void removeQS(long task_id){
        synchronized(map){
            QuartzScheduleInfoMap.map.remove(task_id);
        }
    }
