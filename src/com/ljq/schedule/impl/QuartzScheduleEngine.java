...
import com.ljq.schedule.IScheduleEngine;
...
//        	CronTrigger trigger = new CronTrigger(key,null,info.getTimeExp());
            ...
            details.getJobDataMap().put(ScheduleInfo.FLAG_TASK_ID,info.getTask_id());
            ...
            // 以任务类型记录调度任务信息
            QuartzScheduleInfoMap.addQS(info.getTask_id(),info.getTask_type());
            log.debug("已存储task_id为["+info.getTask_id()+"]调度任务");

            log.debug("添加["+info.getTask_id()+"]调度计划到调度器");
    ...
    public boolean isTaskPlanExsit(ScheduleInfo info) throws Exception {
        ...
        if(null == scheduler.getTrigger(new TriggerKey(info.getTask_id()+"", null))){
            return false;
        }else{
            return true;
        }
